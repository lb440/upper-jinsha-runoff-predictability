...
# ==================== 配置 ====================
def parse_args():
    parser = argparse.ArgumentParser(description="Run the LSTM model with a fixed 90-day window.")
...
kim = ARGS.kim
save_path = os.path.normpath(ARGS.save_path or f"C:/LSTM-{HORIZON}")
param_path = ARGS.param_path or f"C:/LSTM-BO-{HORIZON}/lstm_best_params.json"
...
if resolved_device == "cuda" and not torch.cuda.is_available():
    raise RuntimeError("CUDA was requested for LSTM.py but no GPU is available.")
...
# ==================== 保存模型 ====================
model_save_path = os.path.join(save_path, "best_lstm_model.pth")
...
ax.set_title(f"LSTM验证集预测结果：Horizon={HORIZON}天，NSE={val_metrics['NSE']:.4f}")
ax.legend()
ax.grid(alpha=0.3)
...
save_json(
    os.path.join(save_path, "lstm_run_metadata.json"),
    {
        "model_name": "LSTM",
        "run_type": "main",
...
plt.suptitle(f"LSTM汛期洪水性能对比 (H={HORIZON}d)", fontsize=14)
plt.tight_layout()

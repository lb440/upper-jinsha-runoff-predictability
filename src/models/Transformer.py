...
# ==================== 配置 ====================
def parse_args():
    parser = argparse.ArgumentParser(description="Run the Transformer model with a fixed 90-day window.")
...
kim = ARGS.kim
save_path = os.path.normpath(ARGS.save_path or f"C:/Transformer-{HORIZON}")
param_path = ARGS.param_path or f"C:/Transformer-BO-{HORIZON}/transformer_best_params.json"
...
if resolved_device == "cuda" and not torch.cuda.is_available():
    raise RuntimeError("CUDA was requested for Transformer.py but no GPU is available.")
...
class TransformerNet(nn.Module):
    def __init__(
        ...
        self.pos_encoder = PositionalEncoding(d_model, max_len=kim)
...
ax.set_title("Transformer正式模型训练过程")
ax.legend()
ax.grid(alpha=0.3)
...
ax.set_title(
    f"Transformer验证集预测结果：Horizon={HORIZON}天，NSE={val_metrics['NSE']:.4f}"
)
ax.legend()
ax.grid(alpha=0.3)
...
flood_save_dir = save_flood_analysis(
    ...
    run_label="Transformer",
)
save_json(
    os.path.join(save_path, "transformer_run_metadata.json"),
    {
...
for year in flood_years:
    ...
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("=" * 60 + "\n")
        f.write(f"Transformer 无Lag 汛期洪水性能分析\n")
...
with open(summary_txt_path, "w", encoding="utf-8") as f:
    f.write("=" * 60 + "\n")
    f.write("Transformer 无Lag 汛期洪水性能汇总\n")
...
plt.suptitle(f"Transformer汛期洪水性能对比 (H={HORIZON}d)", fontsize=14)
plt.tight_layout()
